    ...
    for root, dirs, files in os.walk(input_dir):
        ...
        for file in files:
            if file.endswith(".jpg"):
                ...
                category = os.path.basename(root)  # Kategoria tekstury to nazwa bieżącego katalogu

                # Utwórz katalog docelowy, jeśli nie istnieje
                output_category_dir = os.path.join(output_dir, category)
                os.makedirs(output_category_dir, exist_ok=True)

                # Wycinaj próbki z obrazu i zapisuj do odpowiedniego katalogu
    ...
    for root, dirs, files in os.walk(input_dir):
        for file in files:
            if file.endswith(".jpg"):
                ...
                # Oblicz macierz zdarzeń szarości dla danego obrazu
                glcm = mhtex.haralick(image.astype(np.uint8))

                # Utwórz słownik cech dla każdej próbki
                features = {'file': file, 'category': os.path.basename(root).split('_')[0]}  # Użyj części nazwy katalogu jako kategorii

                # Oblicz wybrane cechy tekstury dla różnych odległości i kątów
...
def classify_features(features_file, test_size=0.2):
    ...
    # Usuń atrybut nieliczbowy 'file'
    df = df.drop('file', axis=1)

    X = df.drop('category', axis=1)
    y = df['category']

    # Przekształć etykiety na liczby całkowite
    ...
    # Przekształć przewidziane etykiety z powrotem na oryginalne etykiety
